...
<template>
  <div class="transactionPanel">
    <div class="controls">
      ...
      <div class="buttons">
        <b-button variant="black" v-on:click="setTransactionFilter('WEEK')">Last 7 days</b-button>
        <b-button variant="black" v-on:click="setTransactionFilter('MONTH')">Last 30 days</b-button>
        <b-button variant="black" v-on:click="setTransactionFilter('YEAR')">Last Year</b-button>
        <b-button variant="black" @click="sortByAmount()">
          <svg v-if="amountSorting" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
        ...
        <b-button variant="black" @click="sortByDate()">
          <svg v-if="dateSorting" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            class="bi bi-sort-down" viewBox="0 0 16 16">
            <path
              d="M3.5 2.5a.5.5 0 0 0-1 0v8.793l-1.146-1.147a.5.5 0 0 0-.708.708l2 1.999.007.007a.497.497 0 0 0 .7-.006l2-2a.5.5 0 0 0-.707-.708L3.5 11.293V2.5zm3.5 1a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zM7.5 6a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1h-5zm0 3a.5.5 0 0 0 0 1h3a.5.5 0 0 0 0-1h-3zm0 3a.5.5 0 0 0 0 1h1a.5.5 0 0 0 0-1h-1z" />
          </svg>
    ...
    <div class="card transactions">
      <TransactionTabWidget v-for="(transaction, index) in filteredTransactions" :key="index" :transaction="transaction"
        :accounts="accounts" />
    </div>
  </div>
</template>
...
export default {
  name: "TransactionTab",
  components: { TransactionTabWidget },
  data() {
    return {
      input: null,
      transactions: null,
  ...
  mounted() {
    this.getTransactionsAndAccounts();
  },
  methods: {
    async getTransactionsAndAccounts() {
    ...
    setTransactionFilter(filter) {
      ...
      switch (filter) {
        case 'WEEK':
          const weekAgo = currentDate.clone().subtract(7, 'days');
          filteredTransactions = this.transactions.filter(transaction => {
            const executionDate = moment(transaction.dateOfExecution, 'DD-MM-yyyy HH:mm:ss');
            return executionDate.isBetween(weekAgo, currentDate);
          });
          break;
        case 'MONTH':
          const monthAgo = currentDate.clone().subtract(30, 'days');
          filteredTransactions = this.transactions.filter(transaction => {
            const executionDate = moment(transaction.dateOfExecution, 'DD-MM-yyyy HH:mm:ss');
            return executionDate.isBetween(monthAgo, currentDate);
          });
          break;
        case 'YEAR':
          const yearAgo = currentDate.clone().subtract(1, 'year');
          filteredTransactions = this.transactions.filter(transaction => {
            const executionDate = moment(transaction.dateOfExecution, 'DD-MM-yyyy HH:mm:ss');
            return executionDate.isBetween(yearAgo, currentDate);
          });
          break;
        default:
          break;
